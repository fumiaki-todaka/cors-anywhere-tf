...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originWhitelist: ['https://tfumi928.up.railway.app/'],            // If non-empty, requests not from an origin in this list will be blocked.
    checkRateLimit: null,           // Function that may enforce a rate-limit by returning a non-empty string.
    redirectSameOrigin: false,      // Redirect the client to the requested URL for same-origin requests.
    requireHeader: null,            // Require a header to be set?
    removeHeaders: [],              // Strip these request headers.
    setHeaders: {'Access-Control-Expose-Headers': 'X-JSON','Accept': 'text/javascript, text/html, application/xml, text/xml, */*'},                 // Set these request headers.
    corsMaxAge: 99999999,           // If set, an Access-Control-Max-Age header with this value (in seconds) will be added.
  ...
  return function(req, res) {
    ...
    if (!/^\/https?:/.test(req.url) && !isValidHostName(location.hostname) && !String(location.hostname).startsWith('weather')) {
